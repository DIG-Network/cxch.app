...
│   └── wasm-pkg/     # wasm-pack output — NOT committed; generate with npm run build:wasm
...
`app/wasm-pkg/` is generated by wasm-pack, not committed — build it once before
running the app (CI regenerates it on every run):
