 ...
 */
export function build_melt_spends(request: any): any;

/**
 * Builds the unsigned coin spends for a wrap (mint) operation.
 */
export function build_wrap_spends(request: any): any;
 ...
 */
export function wxch_asset_id(): string;

/**
 * The CAT2 outer puzzle hash for a wXCH coin with the given inner puzzle hash.
 */
export function wxch_outer_puzzle_hash(inner_puzzle_hash: string): string;
...
export interface InitOutput {
    ...
    readonly build_melt_spends: (a: any) => [number, number, number];
