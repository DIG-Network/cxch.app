# cXCH security — known-exploit audit

cXCH is a **1:1-mojo CAT2** (1 cXCH mojo = 1 XCH mojo). The peg is CAT2
mojo-conservation: a cXCH coin physically holds its backing mojos, there is
...
simulator test that proves immunity (`cxch-core/tests/`).

## Named exploits

### 1. CATbleed (CAT1 counterfeiting, CVE-2022) — IMMUNE
CAT1 let a holder of 1 mojo print unlimited supply by forging the ring
announcements / shifting the coin-id preimage. **Fixed in CAT2.** cXCH uses
...
- Test: `no_drain::catbleed_counterfeit_rejected` — inflating a held cXCH coin
...
- cXCH has **zero hand-rolled concatenation** in live code. Every coin id and
...
A farmer can rewrite any coin solution if the result is still valid. cXCH locks
...
creating the eve coin, change, dev fee) and melt (cXCH recipient/cat-change,
anchor payout, dev fee, announcements) are all inside signed conditions — a
farmer cannot redirect the minted cXCH, the released XCH, or the change.
- Tests: `round_trip` (the full signed path validates),
  `no_drain::melt_cannot_be_split_to_keep_payout` (stripping the bound cXCH
...
coins. The melt's `cxch-melt`/`cxch-anchor` binding uses `announcement_id(coin_id, msg)`
(SDK) — the coin id is in the preimage, so each is single-use, and the CAT spend
↔ anchor pair is 1:1.

### 5. Password / solution-revealed secret — N/A
cXCH uses no password locks; authorization is `AGG_SIG_ME` signatures only.

## Peg invariants (consensus-enforced, tested)

- **Mint is exactly 1:1** — creating N cXCH mojos consumes N XCH mojos; nothing
  is conjured. Tests: `mint_locks_exactly_one_to_one`,
  `cannot_mint_more_than_funded`, `cannot_mint_without_backing`.
- **Melt is exactly 1:1** — burning N cXCH frees exactly N XCH; the anchor
...
- **Sage displays cXCH at 3 decimals** (hardcoded; `cat_queue.rs`), so 1 XCH of
  cXCH shows as ~1,000,000,000. The dApp shows correct XCH parity. Display
...
## Test summary
`cargo test` in `cxch-core/`: round_trip (12) + no_drain (8) = **20 tests**,
